/**
 * Migration 021: Backfill projection_pending_approvals from existing activity events.
 *
 * Backfills the projection_pending_approvals table from historical
 * projection_thread_activities and orchestration_events rows, and then
 * recalculates the derived counts on projection_threads. This corrects any
 * gaps left by earlier projector runs that pre-dated the pending-approvals
 * projector or the stale-approval cleanup logic.
 *
 * Also adds the latest_user_message_at column to projection_threads if it does
 * not yet exist, then backfills it from projection_thread_messages.
 */
...
export default Effect.gen(function* () {
  ...
  // Step 0: Add derived-count columns to projection_threads if not present.
  yield* sql`
  ...
  // Step 1: Insert any approval.requested activities that are not yet tracked.
  yield* sql`
  ...
  // Step 2: Resolve approvals that have a matching approval.resolved activity.
  yield* sql`
  ...
  // Step 3: Resolve approvals that have a matching thread.approval-response-requested event.
  yield* sql`
  ...
  // Step 4: Resolve stale approvals — those where the provider tried to respond
  // to an approval that no longer existed (indicated by the error detail text).
  yield* sql`
  ...
  // Step 5: Recalculate derived counts on projection_threads.
  yield* sql`
